...
const Oneblog = (props) => {
  ...
  const fetchcommentstotal = async () => {
    await axios
      .get(
        "http://localhost:2600/comment/getcommentcount/" + `${props.name._id}`
      )
      .then((result) => {
  ...
  const fetchtotallikes = async () => {
    await axios
      .get("http://localhost:2600/comment/totallikes/" + `${props.name._id}`)
      .then((result) => {
  ...
  const change = () => {
    axios
      .post("http://localhost:2600/comment/updatelikes/" + `${props.name._id}`)
      .then((result) => {});
    fetchtotallikes();
    setstate(false);
  };
          ...
          <div className="g-post-ranking">
            {state == true ? (
              <a
                ...
                className="likes"
              >
                <i id="fa" className="fa fa-heart-o" />
                {likes} likes
              </a>
            ) : (
              <a title className="likes">
                <i id="fa" className="fa fa-heart" />
                {likes} likes
              </a>
            )}
            <a
