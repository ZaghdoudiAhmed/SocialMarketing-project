...
import Editorcomponent from '../RoomChat/Editorcomponent';
import Courbe from '../RoomChat/Courbe';
...
socket.on("createMessage", (message, userName) => {
  ...
  messages.innerHTML = messages.innerHTML+`${userName == user ? `<li class="me"><figure><img src="images/resources/userlist-1.jpg" alt /></figure><p class="dt">${message}</p></span><p class="df" >send by ${userName}</p></li>`:
...
<div className="container-fluid">
   ...
   <Box className="top-bannerr" sx={{height: '100vh' }} >
        ...
        <Box spacing={18} sx={{ bgcolor: 'white', height: '80vh',padding:2,display: 'flex' }} boxShadow={10} borderRadius={3}>
        <Box sx={{ bgcolor: '#EDF0F4', height: '75vh',width : '45vh',}} borderRadius={1} >
        <div className="head">
 <p className="design"> Group Chat</p>

         
        <Button onClick={affiche_msg} size="small" className ="butt">Messages</Button>
        <Button size="small" onClick={affiche_participant} className ="butt">Participants</Button>
        </div>
        <Divider    sx={{margin: 1}}/>
  ...
  <div className="message-text-container">
												<span className="form">
  <textarea id="chat_message" placeholder="write your message" defaultValue={""} />
  <button className="do" id="send" title="send"><i className="fa fa-paper-plane" /></button>
</span>
